...
| S2  | 🔴 high     | `fastify`                   | 5.12.0   | ≥ 5.7.2                     | [GHSA-jx2c-rxcm-jvmq](https://github.com/advisories/GHSA-jx2c-rxcm-jvmq)                  | Bypass de validation body via tab dans Content-Type. **Résolu** : mis à jour en 5.12.0 (chaque handler revalide aussi avec `zod.safeParse`).                                                                           |
| S3  | 🔴 high     | `find-my-way` (via fastify) | 9.8.0    | ≥ 9.7.0                     | [GHSA-c96f-x56v-gq3h](https://github.com/advisories/GHSA-c96f-x56v-gq3h)                  | DDoS HTTP/2. **Résolu** : corrigé via la mise à jour de `fastify@5.12.0` (incluant `find-my-way@9.8.0` ; HTTP/2 n'est par ailleurs jamais activé).                                                                     |
| S4  | 🟡 moderate | `fastify`                   | 5.12.0   | ≥ 5.8.3                     | [GHSA-444r-cwp2-x5xf](https://github.com/advisories/GHSA-444r-cwp2-x5xf)                  | `request.protocol`/`host` falsifiables. **Résolu** : mis à jour en 5.12.0 (l'app ne construit par ailleurs aucune URL depuis ces champs).                                                                              |
| S5  | 🟢 low      | `fastify`                   | 5.12.0   | ≥ 5.7.3                     | [GHSA-mrq3-vjjr-p77c](https://github.com/advisories/GHSA-mrq3-vjjr-p77c)                  | DoS mémoire via `sendWebStream`. **Résolu** : mis à jour en 5.12.0 (serveur localhost mono-utilisateur, aucune exposition réseau).                                                                                     |

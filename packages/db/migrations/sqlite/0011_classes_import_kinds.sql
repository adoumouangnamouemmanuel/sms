-- Phase 4.2/4.3: subjects, classrooms and class-subject assignments are
-- importable alongside people, so the confirmed-import audit table must accept
-- the new kinds.
--
-- Rollback rationale: Irreversible. Rolling back would drop class import batch
-- history, violating audit retention requirements.
CREATE TABLE `import_batch_new` (
	...
	`deleted_at` text,
	CONSTRAINT `import_batch_kind_check` CHECK(`kind` in ('STUDENTS', 'TEACHERS', 'GUARDIANS', 'SUBJECTS', 'CLASSROOMS', 'CLASS_SUBJECTS')),

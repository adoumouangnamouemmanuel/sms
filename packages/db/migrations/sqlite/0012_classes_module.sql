-- Phase 4.2: register the CLASSES module.
-- Rebuilds school_module_config so the module_name CHECK accepts 'CLASSES'
-- (SQLite cannot alter a CHECK constraint in place), preserving all rows and
-- indexes, then backfills the enabled CLASSES row for every school that
-- lacks it so already-setup schools see the module too.
--
-- Rollback rationale: Irreversible. Rolling back would remove the CLASSES
-- module config and drop any data associated with it, which violates data
-- retention rules.
...
CREATE TABLE `__new_school_module_config` (
	...
	`deleted_at` text,
	CONSTRAINT `school_module_config_module_name_check` CHECK(`module_name` in ('SCHOOL_SETUP', 'ACADEMIC_STRUCTURE', 'STUDENTS', 'TEACHERS', 'CLASSES')),
...
WHERE NOT EXISTS (
	...
	WHERE `school_module_config`.`school_id` = `school`.`id`
		AND `school_module_config`.`module_name` = 'CLASSES'
);--> statement-breakpoint

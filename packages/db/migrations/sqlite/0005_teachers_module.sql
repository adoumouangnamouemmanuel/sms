-- Phase 3.3: register the TEACHERS module.
-- Rebuilds school_module_config so the module_name CHECK accepts 'TEACHERS'
...
CREATE TABLE `__new_school_module_config` (
	...
	`deleted_at` text,
	CONSTRAINT `school_module_config_module_name_check` CHECK(`module_name` in ('SCHOOL_SETUP', 'ACADEMIC_STRUCTURE', 'STUDENTS', 'TEACHERS')),
...
WHERE NOT EXISTS (
	...
	WHERE `school_module_config`.`school_id` = `school`.`id`
		AND `school_module_config`.`module_name` = 'TEACHERS'
);--> statement-breakpoint
